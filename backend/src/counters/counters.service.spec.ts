...
describe('CountersService', () => {
  ...
  it('admin sees all counters', async () => {
    ...
    expect(countersRepo.find).toHaveBeenCalledWith({
  ...
  it('staff sees only their assigned counter', async () => {
    countersRepo.find.mockResolvedValue([
      {
        id: 4,
      },
  ...
  it('staff cannot view another staff counter', async () => {
    ...
    await expect(service.findOne(1, staff)).rejects.toThrow(ForbiddenException);
  });

  it('admin can change counter status', async () => {
    const counter = {
      id: 1,
    ...
    countersRepo.save.mockImplementation(async (value) => value);
  ...
  it('assigns a staff user to a counter', async () => {
    const counter = {
      id: 1,
      status: CounterStatus.CLOSED,
      staff: null,
    ...
    usersRepo.findOne.mockResolvedValue(staffUser);

    countersRepo.save.mockImplementation(async (value) => value);

    const result = await service.assignStaff(1, 2);

    expect(result.staff).toEqual(staffUser);
  });

  it('creates counter with selected active services', async () => {
    ...
    countersRepo.create.mockImplementation((value) => value);
    ...
    expect(result.name).toBe('Counter 1');
  });
});
