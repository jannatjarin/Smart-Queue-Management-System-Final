...
describe('CountersController', () => {
  ...
  it('findAll passes current user', async () => {
    await controller.findAll(admin);

    expect(service.findAll).toHaveBeenCalledWith(admin);
  });

  it('findOne passes id and current user', async () => {
    await controller.findOne(1, admin);

    expect(service.findOne).toHaveBeenCalledWith(1, admin);
  });

  it('assignStaff passes id and staffId', async () => {
    await controller.assignStaff(1, {
      staffId: 5,
    });
  ...
  it('updateStatus passes current user', async () => {
    const dto = {
      status: CounterStatus.OPEN,
    };

    await controller.updateStatus(1, dto, admin);

    expect(service.updateStatus).toHaveBeenCalledWith(1, dto, admin);
  });
});
