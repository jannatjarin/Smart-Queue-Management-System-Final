...
describe('AuthService', () => {
  ...
  let usersService: {
    ...
    updateResetTokenVersion: jest.Mock;
  };
  ...
  beforeEach(() => {
    usersService = {
      ...
      updateResetTokenVersion: jest.fn(),
    };
  ...
  it('always registers public users as CUSTOMER', async () => {
  ...
  it('returns access and refresh tokens after login', async () => {
  ...
  it('refresh uses the current database role', async () => {
  ...
  it('forgot password does not reveal unknown email', async () => {
    usersService.getUserByEmail.mockResolvedValue(null);
  ...
  it('creates a new reset token version', async () => {
    usersService.getUserByEmail.mockResolvedValue({
      id: 4,
      email: 'user@test.com',
      resetTokenVersion: 2,
    ...
    expect(usersService.updateResetTokenVersion).toHaveBeenCalledWith(4, 3);
  ...
  it('rejects an already used reset token', async () => {
    ...
    await expect(
      service.resetPassword('old-token', 'newpassword'),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it('resets password and invalidates token', async () => {
    ...
    expect(usersService.updateResetTokenVersion).toHaveBeenCalledWith(2, 4);
