...
export class TicketsService {
  ...
  private async findStaffCounter(userId: number): Promise<Counters | null> {
    return this.countersRepository.findOne({
      where: {
        staff: {
          id: userId,
        },
  ...
  private async getStaffCounter(userId: number): Promise<Counters> {
    const counter = await this.findStaffCounter(userId);
  ...
  private async calculateEstimatedWait(
    ...
    const peopleAhead = await this.ticketsRepository.count({
      where: {
        queue: {
          id: ticket.queue.id,
        },
        ...
        issuedAt: LessThan(ticket.issuedAt),
      },
    });

    const openCounters = await this.countersRepository
      .createQueryBuilder('counter')
      .innerJoin('counter.services', 'service')
      .where(
    ...
    return Math.ceil(peopleAhead / openCounters) * ticket.service.estimatedTime;
  }

  async create(dto: CreateTicketDto, currentUser: CurrentUserPayload) {
    const result = await this.dataSource.transaction(async (manager) => {
      const servicesRepository = manager.getRepository(Services);

      const queuesRepository = manager.getRepository(Queues);

      const ticketsRepository = manager.getRepository(Tickets);
      ...
      const queue = await queuesRepository
      ...
      const existingActiveTicket = await ticketsRepository.count({
        where: {
          user: {
            id: currentUser.id,
          },
      ...
      if (existingActiveTicket > 0) {
        throw new BadRequestException(
          'You already have an active ticket in this queue',
        );
      }

      const waitingAhead = await ticketsRepository.count({
        where: {
          queue: {
            id: queue.id,
          },
      ...
      await queuesRepository.save(queue);

      const ticketNumber = `Q${queue.id}-${String(
      ...
      const saved = await ticketsRepository.save(ticket);

      const openCounters = await manager
        .getRepository(Counters)
        .createQueryBuilder('counter')
        .innerJoin('counter.services', 'counterService')
        .where(
      ...
      const estimatedWaitMinutes =
        openCounters == 0
          ? null
          : Math.ceil(waitingAhead / openCounters) * service.estimatedTime;
  ...
  async findAll(
    ...
    if (currentUser.role == Role.STAFF) {
      const counter = await this.findStaffCounter(currentUser.id);
      ...
      if (queueId) {
        ...
        if (!serviceIds.includes(queue.service.id)) {
  ...
  async findMyTickets(userId: number) {
    ...
    return Promise.all(
      tickets.map(async (ticket) => ({
        ...
        estimatedWaitMinutes: await this.calculateEstimatedWait(ticket),
      })),
    );
  }
  ...
  async findOne(id: number, currentUser: CurrentUserPayload) {
    const ticket = await this.ticketsRepository.findOne({
      ...
      relations: ['user', 'service', 'queue', 'counter', 'counter.staff'],
    });
    ...
    if (currentUser.role == Role.STAFF) {
      const counter = await this.getStaffCounter(currentUser.id);

      const supportedServiceIds = counter.services.map((service) => service.id);

      if (!supportedServiceIds.includes(ticket.service.id)) {
        throw new ForbiddenException(
  ...
  async callNext(
    ...
    if (queue.status != QueueStatus.OPEN) {
      throw new BadRequestException('This queue is currently closed');
    }

    const counter = await this.getStaffCounter(staffUser.id);
    ...
    const supportsService = counter.services.some(
      (service) => service.id == queue.service.id,
    );
    ...
    const activeCalledTicket = await this.ticketsRepository.count({
      where: {
        counter: {
          id: counter.id,
        },
  ...
  async complete(
    ...
    const ticket = await this.ticketsRepository.findOne({
      ...
      relations: ['user', 'counter', 'counter.staff'],
    });
    ...
    if (currentUser.role == Role.STAFF) {
      if (!ticket.counter || ticket.counter.staff?.id != currentUser.id) {
        throw new ForbiddenException(
  ...
  async cancel(id: number, currentUser: CurrentUserPayload): Promise<Tickets> {
    ...
    if (currentUser.role == Role.STAFF) {
      throw new ForbiddenException('Staff can not cancel tickets');
    }
