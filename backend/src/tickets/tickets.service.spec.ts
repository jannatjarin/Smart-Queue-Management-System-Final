...
describe('TicketsService', () => {
  ...
  let mailService: {
    sendTicketReadyEmail: jest.Mock;
  };
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(Queues),

          useFactory: mockRepository,
        },

        {
          provide: getRepositoryToken(Counters),
  ...
  it('returns customer ticket history', async () => {
    ...
    const result = await service.findMyTickets(1);

    expect(result[0].estimatedWaitMinutes).toBe(0);
  });

  it('staff can call next ticket only from supported open queue', async () => {
    ...
    countersRepo.findOne.mockResolvedValue({
      id: 4,
      name: 'Counter 1',
    ...
    ticketsRepo.count.mockResolvedValue(0);
    ...
    ticketsRepo.save.mockImplementation(async (value) => value);

    const result = await service.callNext(1, staff);

    expect(result.status).toBe(TicketStatus.CALLED);

    expect(result.counter?.id).toBe(4);

    expect(mailService.sendTicketReadyEmail).toHaveBeenCalled();

    expect(notificationsService.create).toHaveBeenCalled();
  });

  it('staff cannot complete another counter ticket', async () => {
    ...
    await expect(service.complete(1, staff)).rejects.toThrow(
      ForbiddenException,
    );
  });

  it('admin can complete a called ticket', async () => {
    const ticket = {
      id: 1,
      ticketNumber: 'Q1-001',
    ...
    const result = await service.complete(1, admin);
  ...
  it('customer can cancel own waiting ticket', async () => {
    const ticket = {
      id: 1,
      ticketNumber: 'Q1-001',
  ...
  it('customer cannot cancel someone else ticket', async () => {
    ticketsRepo.findOne.mockResolvedValue({
      id: 1,
      status: TicketStatus.WAITING,
    ...
    await expect(service.cancel(1, customer)).rejects.toThrow(
      ForbiddenException,
    );
  });

  it('staff cannot cancel tickets', async () => {
    ticketsRepo.findOne.mockResolvedValue({
      id: 1,
      status: TicketStatus.WAITING,
    ...
    await expect(service.cancel(1, staff)).rejects.toThrow(ForbiddenException);
  });

  it('admin cannot cancel completed ticket', async () => {
    ticketsRepo.findOne.mockResolvedValue({
      id: 1,
      status: TicketStatus.COMPLETED,
