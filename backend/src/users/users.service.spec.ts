...
describe('UsersService', () => {
  ...
  let countersRepo: ReturnType<typeof mockCountersRepository>;
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
...

        {
          ...
          useFactory: mockCountersRepository,
        },
      ],
    }).compile();
  ...
  it('creates and saves a user', async () => {
    ...
    usersRepo.create.mockReturnValue(entity);

    usersRepo.save.mockResolvedValue(entity);
  ...
  it('gets user by email including password', async () => {
    const user = {
      id: 1,
      email: 'a@test.com',
    };
  ...
  it('updates profile and returns fresh user', async () => {
    const oldUser = {
      id: 1,
      fullName: 'Old',
    };
  ...
  it('updates reset token version', async () => {
    await service.updateResetTokenVersion(1, 3);
  ...
  it('admin can promote customer to staff', async () => {
    usersRepo.findOne
    ...
    const result = await service.updateRole(5, Role.STAFF, 1);

    expect(usersRepo.update).toHaveBeenCalledWith(5, {
      role: Role.STAFF,
    });
  ...
  it('admin can promote another user to admin', async () => {
    ...
    const result = await service.updateRole(5, Role.ADMIN, 1);
  ...
  it('admin cannot demote themselves', async () => {
    ...
    await expect(service.updateRole(1, Role.CUSTOMER, 1)).rejects.toThrow(
      BadRequestException,
    );
  });

  it('builds user search role sorting and pagination query', async () => {
    const qb = {
    ...
    const result = await service.findAll('john', Role.STAFF, 'ASC', 2, 5);
